...
private func printingRootViewController() -> UIViewController? {
...
public class PrintJob: UIPrintPageRenderer, UIPrintInteractionControllerDelegate {
    ...
    func convertHtml(_ data: String, withPageSize rect: CGRect, andMargin margin: CGRect, andBaseUrl baseUrl: URL?) {
        guard let viewController = printingRootViewController() else {
            printing.onHtmlError(printJob: self, error: "Unable to find root view controller")
            return
        }
